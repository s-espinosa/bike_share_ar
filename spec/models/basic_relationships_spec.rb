...
describe 'Basic Relationships: ' do
  ...
  describe "Class and scope methods" do
    ...
    xit 'Use a class method to find all days with no precipitation' do
      expect(Condition.no_rain_class).to eq(Condition.where(precipitation: 0))
    end

    xit "Use a scope to find all days with no precipitation" do
      expect(Condition.no_rain_scope).to eq(Condition.where(precipitation: 0))
    end

    xit "Use a class method to find all the stations with dock counts of 15" do
      expect(Station.docks_of_15_class).to eq(Station.where(dock_count: 15))
    end

    xit "Use a scope to find all the stations with dock counts of 15" do
      expect(Station.docks_of_15_scope).to eq(Station.where(dock_count: 15))
    end

    xit "Use a class method to find all the stations in Brooklyn" do
      brooklyn = City.find_by(name: "Brooklyn")
      expect(Station.brooklyn_class).to eq(Station.where(city: brooklyn))
    end

    xit "Use a scope to find all the stations in Brooklyn" do
      brooklyn = City.find_by(name: "Brooklyn")
  ...
  describe "Class calculations on the model" do
    ...
    xit "Calculates the average number of docks for all stations" do
      expect(Station.average_dock_count).to eq(Station.average(:dock_count))
    end

    xit "Calculates the average duration of a trip" do
  ...
  describe "Instance calculations on the model" do
    ...
    xit "Calculates the average number of docks in a city" do
      brooklyn = City.find_by(name: "Brooklyn")
    ...
    xit "Calculates the average duration of a trip with a given subscription type" do
    ...
    xit "Calculates the average duration of a trip started on a given day" do
    ...
    xit "Calculates the average duration of a trip started at a particular station" do
